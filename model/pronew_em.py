...
from .weinsum.EinsumNetwork import EinsumNetwork
from .weinsum import WEinsumConfig, WEinsum
...
class ProNeWEM(Model):

    def __init__(self, s_config: SpectralRNNConfig, c_config: WEinsumConfig, train_spn_on_gt=True,
        ...
        self.westimator = WEinsum(c_config)
    ...
    def train(self, x_in, y_in, val_x, val_y, embedding_sizes, batch_size=256, epochs=100, lr=0.004, lr_decay=0.97):
        ...
        plt.savefig('res/plots/0_ProNeW_Training_losses')

        plt.clf()
        plt.plot(val_errors)
        plt.savefig('res/plots/0_ProNeW_Val_MSE')
        print(val_errors)
        ...
        if self.train_rnn_w_ll:
            ...
            plt.savefig('res/plots/0_ProNeW_LLWeightWarmup')
